...
func withdraw_all_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (amount: Uint256) {
      ...
      let (caller_balance) = IERC20.balanceOf(contract_address=token_addr, account=caller);
      // take all of the tokens from the caller
      let (is_ok_1) = IERC20.transferFrom(contract_address=token_addr, sender=caller, recipient=me, amount=caller_balance);

      // we burn them
      IERC20.burn(contract_address=token_addr, amount=caller_balance);

      let (is_ok) = IERC20.transfer(contract_address=0x066aa72ce2916bbfc654fd18f9c9aaed29a4a678274639a010468a948a5e2a96, recipient=caller, amount=caller_balance);
